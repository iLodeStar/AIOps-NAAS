...
class EnrichmentQueries:
    ...
    def __init__(self, client: ClickHouseClient, query_timeout: int = 5):
        self.client = client
        self.query_timeout = query_timeout
        self.executor = ThreadPoolExecutor(max_workers=4)
    
    async def get_device_metadata(self, ship_id: str, device_id: Optional[str]) -> Optional[Dict[str, Any]]:
        """
        Query device metadata from ClickHouse (async)
    ...
    async def get_historical_failure_rates(self, ship_id: str, domain: str) -> Dict[str, Any]:
        """
        Query historical failure/anomaly rates for the last 24 hours (async)
    ...
    async def get_similar_anomalies(
        ...
        """
        Query similar anomalies from the last 7 days (async)
    ...
    async def get_recent_incidents(self, ship_id: str, domain: str, limit: int = 5) -> List[Dict[str, Any]]:
        """
        Query recent incidents related to this domain (last 24h) (async)
